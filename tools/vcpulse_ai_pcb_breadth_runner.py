...
def build_groups(theme_db,family_mode="max"):
    ...
    for can,by_code in raw_groups.items():
        ...
        for code,metas in by_code.items():
            ...
            # Candidate: retain one limited breadth signal only for AI PCB.
            if family_mode=="ai_pcb_breadth25" and can=="AI PCB":
                w1,m1=weighted[0]
            ...
            if family_mode in ("max","ai_pcb_breadth25","ai_pcb_current_reference"):
                w,m=weighted[0]
...
def main():
    ...
    ap.add_argument("--family-mode",choices=["max","ai_pcb_breadth25","ai_pcb_current_reference"],default="max")
    args=ap.parse_args()
