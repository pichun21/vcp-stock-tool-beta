"""
VCPulse Theme Historical Runner v0.9.1 Retry-safe long history
Modes:
...
def fetch_finmind(code,start_date,end_date,token=""):
    ...
    for attempt in range(1,5):
        try:
            r=requests.get(FINMIND,params=params,headers=headers,timeout=90)
            r.raise_for_status()
            j=r.json()
            ...
            if attempt==4:
                raise
            wait=attempt*5
            print(f"FinMind request failed for {code} (attempt {attempt}/4): {e}; retry in {wait}s")
            time.sleep(wait)
    raise last_error
...
def run_finmind(theme_db,start_test,end_test,cache_dir,token="",max_days=40):
    ...
    if max_days and len(calendar)>max_days:
        calendar=calendar[-max_days:]
    if max_days and len(calendar)<max_days:
        print(f"WARNING: requested validation produced only {len(calendar)} trading days; target is {max_days}.")
...
def main():
    ...
    ap.add_argument("--max-days",type=int,default=40,help="0 = unlimited validation days")
    args=ap.parse_args()

    theme_db=json.loads(Path(args.theme_db).read_text(encoding="utf-8"))
    token=os.environ.get("FINMIND_TOKEN","")
    daily=run_finmind(theme_db,args.start,args.end,args.cache_dir,token,args.max_days)
