...
def build_groups(theme_db,family_mode="current"):
    ...
    for can,by_code in raw_groups.items():
        ...
        for code,metas in by_code.items():
            if family_mode=="current" or can not in FAMILY_CANONICAL or len(metas)<=1:
            ...
            if family_mode=="max":
                ...
                out.append((code,mm))
            elif family_mode=="decay":
                mults=(1.0,0.25,0.10)
                total=sum(w*(mults[i] if i<len(mults) else 0.0) for i,(w,_) in enumerate(weighted))
                mm=dict(weighted[0][1]); mm["_weight_override"]=total
                out.append((code,mm))
            else:
...
def main():
    ...
    ap.add_argument("--family-mode",choices=["current","max","decay"],default="current")
    args=ap.parse_args()
