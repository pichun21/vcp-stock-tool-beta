...
def build_groups(theme_db,family_mode="max"):
    ...
    for can,by_code in raw_groups.items():
        ...
        for code,metas in by_code.items():
            ...
            # Diagnostic upper reference only: restore original AI PCB duplicate contribution.
            if family_mode=="ai_pcb_current_reference" and can=="AI PCB":
                out.extend((code,m) for m in metas)
                continue

            # Candidate 1: isolate the third-membership structural signal.
            if family_mode=="triple_indicator" and can=="AI PCB" and len(weighted)>=3:
                w1,m1=weighted[0]
                w3=weighted[2][0]
                mm=dict(m1); mm["_weight_override"]=w1+0.50*w3
                out.append((code,mm))
                continue

            # Candidate 2: balanced breadth, but only for true triple-membership stocks.
            if family_mode=="triple_balanced" and can=="AI PCB" and len(weighted)>=3:
                w1,m1=weighted[0]
                w2=weighted[1][0]; w3=weighted[2][0]
                mm=dict(m1); mm["_weight_override"]=w1+0.50*w2+0.50*w3
                out.append((code,mm))
                continue

            # Everything else remains strict MAX.
            if family_mode in ("max","triple_indicator","triple_balanced","ai_pcb_current_reference"):
                w,m=weighted[0]
...
def main():
    ...
    ap.add_argument("--family-mode",choices=["max","triple_indicator","triple_balanced","ai_pcb_current_reference"],default="max")
    args=ap.parse_args()
