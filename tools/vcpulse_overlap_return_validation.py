...
def main():
    ...
    report={
        "schema":"vcpulse-alpha129-overlap-return-validation",
    ...
    (out/"alpha129_return_validation.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    pd.DataFrame(rows).to_csv(out/"alpha129_return_observations.csv",index=False,encoding="utf-8-sig")

    # Compact markdown report
    lines=[
        "# VCPulse Alpha129 — Overlap Return Validation",
        "",
    ...
    for pair,pv in report["pairwise_rank_matched"].items():
        ...
        lines.append("")
    (out/"alpha129_return_validation.md").write_text("\n".join(lines),encoding="utf-8")
    print(json.dumps({
