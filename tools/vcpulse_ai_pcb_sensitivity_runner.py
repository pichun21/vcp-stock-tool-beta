"""
VCPulse Overlap Long-OOS Runner — Alpha128
Modes:
...
def membership_weight(meta):
    if "_weight_override" in meta:
        return float(meta["_weight_override"])
...
FAMILY_CANONICAL={"重電/強韌電網","AI PCB","AI電力基建","國防航太","低軌衛星"}

def build_groups(theme_db,family_mode="max"):
    amap=theme_db["themeTaxonomy"]["alias_to_canonical"]; raw_groups={}
    for code,s in theme_db["stocks"].items():
        for raw,meta in s.get("themes",{}).items():
            can=amap.get(raw)
            if can: raw_groups.setdefault(can,{}).setdefault(code,[]).append(meta)

    groups={}
    for can,by_code in raw_groups.items():
        out=[]
        for code,metas in by_code.items():
            # Non-audited themes stay unchanged.
            if can not in FAMILY_CANONICAL or len(metas)<=1:
                out.extend((code,m) for m in metas)
                continue

            weighted=sorted(((membership_weight(m),m) for m in metas),key=lambda x:x[0],reverse=True)

            # Reference only: restore original duplicate contribution for AI PCB,
            # while every other audited family remains MAX.
            if family_mode=="ai_pcb_current_reference" and can=="AI PCB":
                out.extend((code,m) for m in metas)
                continue

            # Alpha134 candidates: retain a bounded second-membership breadth signal only for AI PCB.
            if family_mode.startswith("ai_pcb_breadth") and can=="AI PCB":
                coef={"ai_pcb_breadth40":0.40,"ai_pcb_breadth55":0.55,
                      "ai_pcb_breadth70":0.70,"ai_pcb_breadth85":0.85}[family_mode]
                w1,m1=weighted[0]
                w2=weighted[1][0] if len(weighted)>1 else 0.0
                total=w1+coef*w2
                mm=dict(m1); mm["_weight_override"]=total
                out.append((code,mm))
                continue

            # GLOBAL_MAX, and all other audited families under both hybrid modes.
            if family_mode in ("max","ai_pcb_breadth40","ai_pcb_breadth55","ai_pcb_breadth70","ai_pcb_breadth85"):
                w,m=weighted[0]
                mm=dict(m); mm["_weight_override"]=w
                out.append((code,mm))
                continue
...
def run_finmind(theme_db,start_test,end_test,cache_dir,token="",family_mode="current"):
    ...
    forward_end=(pd.Timestamp(end_test)+pd.Timedelta(days=45)).strftime("%Y-%m-%d")
    prices={}
    for i,code in enumerate(codes,1):
        ...
        if cp.exists():
            ...
            df=fetch_finmind(code,fetch_start,forward_end,token)
            df.reset_index().to_csv(cp,index=False)
            time.sleep(.15)
        prices[code]=df
        print(f"[{i}/{len(codes)}] {code} {len(df)} bars")

    # Benchmark = FinMind TaiwanStockTotalReturnIndex, data_id=TAIEX.
    # Persist as TAIEX_TOTAL_RETURN.csv for downstream validation.
    bp=cache/"TAIEX_TOTAL_RETURN.csv"
    try:
        if bp.exists():
            bench=pd.read_csv(bp,parse_dates=["Date"]).set_index("Date")
        else:
            headers={"Authorization":f"Bearer {token}"} if token else {}
            params={
            ...
            rr=requests.get(FINMIND,params=params,headers=headers,timeout=60)
            rr.raise_for_status()
            jj=rr.json()
            if jj.get("status")!=200:
                raise RuntimeError(jj)
            bdf=pd.DataFrame(jj.get("data",[]))
            if bdf.empty:
                raise RuntimeError("TAIEX total-return index returned no rows")
            bdf["date"]=pd.to_datetime(bdf["date"])
            bdf["price"]=pd.to_numeric(bdf["price"],errors="coerce")
            bdf=bdf.dropna(subset=["date","price"]).sort_values("date")
            bench=bdf.rename(columns={"date":"Date","price":"Close"})[["Date","Close"]].set_index("Date")
            bench.reset_index().to_csv(bp,index=False)
        print(f"[benchmark] TAIEX total-return rows={len(bench)}")
    ...
    # Long-OOS: keep the full requested trading calendar; no 20-session truncation.

    groups=build_groups(theme_db,family_mode)
...
def main():
    ...
    ap.add_argument("--family-mode",choices=["max","ai_pcb_breadth40","ai_pcb_breadth55","ai_pcb_breadth70","ai_pcb_breadth85"],default="max")
    args=ap.parse_args()
