"""
VCPulse Theme Historical Runner v0.3 Coverage Audit
Modes:
...
def build_coverage_audit(daily):
    ...
    for day in daily:
        ...
        for g in day.get("themeGateDiagnostics",[]):
            if not g["eligible"] and g["activeRadarMembers"]>0:
...
def run_finmind(theme_db,start_test,end_test,cache_dir,token=""):
    ...
    for d in calendar:
        ...
        for name,members in groups.items():
            gate=theme_gate_diagnostic(name,members,quotes)
...
def main():
    ...
    print(json.dumps({"summary":summary,"coverageAudit":audit},ensure_ascii=False,indent=2))
