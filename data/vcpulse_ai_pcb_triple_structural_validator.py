...
CANDS=("TRIPLE_INDICATOR","TRIPLE_BALANCED")
MODES=("MAX","TRIPLE_INDICATOR","TRIPLE_BALANCED","CURRENT_REF")
...
def main():
    ...
    ap.add_argument("--triple-indicator",required=True); ap.add_argument("--triple-balanced",required=True)
    ap.add_argument("--reference",required=True); ap.add_argument("--price-cache",required=True)
    ap.add_argument("--benchmark-file",required=True); ap.add_argument("--out-dir",default="alpha136_result")
    a=ap.parse_args()
    db=J(a.theme_db); mem=members(db)
    paths={"MAX":a.max,"TRIPLE_INDICATOR":a.triple_indicator,"TRIPLE_BALANCED":a.triple_balanced,"CURRENT_REF":a.reference}
          ...
          for h in H:
            ...
            if d0 in bench.index:
              ...
              if i>=0 and i+h<len(bench):
                p0=float(bench.iloc[i].Close); p1=float(bench.iloc[i+h].Close)
            ...
            for m in paths:
              if rr[m] is not None:
                rows.append({"date":date,"view":view,"rank":rank,"horizon":h,"mode":m,
    ...
    summaries={}; checks={}; details=[]
    for c in ("TRIPLE_INDICATOR","TRIPLE_BALANCED","CURRENT_REF"):
      z=mx.merge(r[r["mode"]==c][keys+["theme","return"]],on=keys)
      z["delta"]=z["return"]-z["return_max"]; z["changed"]=z["theme"]!=z["theme_max"]
      details.append(z.assign(candidate=c))
      s={}
      for view in VIEWS:
        s[view]={}
        for h in H:
      ...
      if c in CANDS:
        ...
        for h in ("10","20"):
          x=s["heat_top1"][h]["up"]
          ck.append({"name":f"heat_up_{h}D_positive","pass":x["mean"] is not None and x["mean"]>0,"value":x})
          y=s["setup_top1"][h]["all"]
          ck.append({"name":f"setup_all_{h}D_nonnegative","pass":y["mean"] is not None and y["mean"]>=0,"value":y})
        changed_dates=int(z[(z["view"]=="heat_top1") & z["changed"]]["date"].nunique())
        ck.append({"name":"changes_at_least_one_heat_top1","pass":changed_dates>0,"value":{"changed_dates":changed_dates}})
        checks[c]=ck

    passing=[c for c in CANDS if all(x["pass"] for x in checks[c])]
    selected=passing[0] if passing else None
    report={"schema":"vcpulse-alpha136-triple-structural-breadth","days":len(dates),
      "benchmark_loaded":not bench.empty,"selection_rule":"TRIPLE_INDICATOR priority, then TRIPLE_BALANCED; never choose by highest return",
      "gate":"PASS" if selected else "HOLD","selected":selected,"checks":checks,
      "summary_vs_global_max":summaries,"production":"UNCHANGED"}

    out=Path(a.out_dir);out.mkdir(parents=True,exist_ok=True)
    (out/"alpha136_triple_structural_validation.json").write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding="utf-8")
    pd.concat(details,ignore_index=True).to_csv(out/"alpha136_candidate_vs_max.csv",index=False,encoding="utf-8-sig")
    lines=["# VCPulse Alpha136 — Triple-Membership Structural Breadth","",
      f"- Gate: **{report['gate']}**",f"- Selected: **{selected or 'NONE'}**",
      f"- Days: **{len(dates)}**","- Production: **UNCHANGED**","",
    ...
    (out/"alpha136_triple_structural_validation.md").write_text("\n".join(lines),encoding="utf-8")
    print(json.dumps({"gate":report["gate"],"selected":selected,"days":len(dates)},ensure_ascii=False))
